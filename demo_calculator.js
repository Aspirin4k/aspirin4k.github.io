...
function is_correct(str)
{
	...
	for (var i=0; i<str.length; i++)
	{
		...
		{
			par--;
			// Если закрылось больше скобок, чем открылось
			if (par<0) return false;
		}
		// Если слева от оператора открывающаяся скобка, то это ошибка (Исключение - унарный минус)
		if (operators.has(str[i]) && (str[i]!='-') && ((str[i-1]=='(') || (i==0))) return false;
		// Если справа от оператора закрывающаяся скобка, то это ошибка
